...
<Note>
`tools_list_dictionary` is one of three separate tool mechanisms the API exposes. The other two — the built-in `auto_search`/`web_scraper` tools (`tools_enabled`) and the autonomous looper's tools (`selected_tools`, only used when `max_loops` is `"auto"`) — are documented on the [Tools](/docs/documentation/capabilities/tools) page.
</Note>
...
Use `tools_list_dictionary` in the agent config to define available tools using the OpenAI function calling schema. This is for *your own* functions — for the API's built-in `auto_search`/`web_scraper` tools or the autonomous looper's tools, see the [Tools](/docs/documentation/capabilities/tools) page instead.
...
|-------|------|-------------|
| `function.name` | `string` | The name of the tool the model wants to call |
| `function.arguments` | `string` | A JSON-encoded string containing the tool call arguments |
...
// 2. Extract tool calls from the response
const agentOutput = data.outputs[0];
const toolCalls = agentOutput.content; // Array of tool call objects
...
for (const agentOutput of data.output) {
    const agentName = agentOutput.role;
    let content = agentOutput.content;

    // Swarm content is a string — parse it into an array
    let toolCalls: any[];
    ...
    for (const toolCall of toolCalls) {
        ...
        console.log(`[${agentName}] Tool: ${functionName}`);
        console.log(`  Arguments:`, args);
    }
}
```
...
<Accordion title="How is tools_list_dictionary different from tools_enabled and selected_tools?">
`tools_list_dictionary` defines your own custom functions using an OpenAI-style schema — the model decides when to call them, and your application executes them.

`tools_enabled` (on the request body, alongside `agent_config`) turns on the API's two built-in tools, `auto_search` and `web_scraper`, which the API executes for you.

`selected_tools` (in `agent_config`) restricts the autonomous execution loop's own tool set (planning, file, and sub-agent tools), and only applies when `agent_config.max_loops` is `"auto"`.

All three can be combined on the same request. See the [Tools](/docs/documentation/capabilities/tools) page for the built-in and autonomous-loop mechanisms.
</Accordion>
</AccordionGroup>
