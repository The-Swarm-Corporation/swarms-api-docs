...
<Note>
The `name` and `description` strings above are the literal values the API returns (lowercase, verb-first descriptions) — copy them as-is rather than re-casing them, since your gating logic may match on exact string.
</Note>
