...
Each sub-agent receives a unique ID in the format `sub-agent-{uuid}` (e.g., `sub-agent-a1b2c3d4`) and is stored in the coordinator's internal cache for reuse. A sub-agent inherits the coordinator's `model_name` and tool list, and its own `max_loops` is capped internally (5 if it has tools, otherwise 1) so it cannot itself recurse into further sub-agent creation.
        ...
        "handoffs": [
            {
                ...
                "system_prompt": "You resolve billing and refund issues.",
                "model_name": "gpt-4.1"
            },
            {
                "agent_name": "Technical-Support",
                "description": "Handles technical product issues",
                "system_prompt": "You debug and resolve technical product issues.",
                "model_name": "gpt-4.1"
            }
        ]
    },
    "task": "A customer was double-charged last month and also cannot log in. Route this to the right specialists and resolve it."
}
```
...
- **Keep the sub-agent count proportional to the task** — there's no hard limit on how many sub-agents `create_sub_agent` can spawn, but each one adds its own token usage and wall-clock time, so more sub-agents means more cost and coordination overhead for the coordinator to manage.
