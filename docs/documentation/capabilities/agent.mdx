...
| `img` | `string` | No | Base64 encoded image for vision-enabled models |
| `imgs` | `List[string]` | No | Multiple base64 encoded images for vision-enabled models |
...
- Single image analysis via `img` parameter (base64 encoded)
- Multiple image analysis via `imgs` parameter (base64 encoded)
...
### Agent with Vision (Image Analysis)

Use the `img` parameter to send a base64 encoded image for analysis with vision-enabled models like `gpt-4o`.
...
<Tabs>
  <Tab title="Python">
    ...
    # Encode your image
    image_base64 = encode_image_to_base64("path/to/your/image.png")

    payload = {
        "agent_config": {
            "agent_name": "Vision Analyst",
            "description": "Expert in analyzing images and visual content",
            "system_prompt": "You are an expert image analyst. Provide detailed, accurate descriptions and insights about images.",
            "model_name": "gpt-4o",  # Use a vision-capable model
            "max_tokens": 4096,
            "temperature": 0.5
        },
        "task": "Analyze this image and describe what you see in detail.",
  ...
  <Tab title="JavaScript">
    ...
    // Encode your image
    const imageBase64 = encodeImageToBase64('path/to/your/image.png');
  ...
  <Tab title="cURL">
    ...
    IMAGE_BASE64=$(base64 -i path/to/your/image.png)
    ...
    curl -X POST "https://api.swarms.world/v1/agent/completions" \
      ...
      -d '{
        ...
        "task": "Analyze this image and describe what you see in detail.",
        "img": "data:image/png;base64,'"$IMAGE_BASE64"'"
      }'
    ```
  </Tab>
</Tabs>
...
<Note>
**Supported image formats**: PNG, JPEG, GIF, and WebP. Make sure to use the correct MIME type in the data URI (e.g., `data:image/jpeg;base64,` for JPEG images).
</Note>
