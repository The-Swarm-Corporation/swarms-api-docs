...
---
<Warning>
Premium: This endpoint is available only on Pro, Ultra, and Premium plans. See <a href="/docs/documentation/resources/pricing">Pricing</a>.
</Warning>
## Overview
...
You define the shape yourself: agents are nodes, `edges` are the arrows. Branches run in
parallel and rejoin wherever two edges meet the same node.
...
| `model_name` | `string` | No | `"claude-sonnet-5"` | AI model to use (e.g., "gpt-4.1", "gpt-4.1-mini", "claude-sonnet-4-20250514") |
| `max_tokens` | `integer` | No | `16000` | Maximum number of tokens the agent can generate. Values below 1 are rejected with a 422 validation error |
| `temperature` | `float` | No | `null` | Randomness control (lower = more deterministic). If omitted, the provider's own default applies |
| `role` | `string` | No | `"worker"` | Agent's role within the swarm |
| `max_loops` | `integer \| string` | No | `1` | Maximum number of times the agent can repeat its task, or `"auto"`. Integer values below 1 are rejected with a 422 validation error |
...
| `dynamic_temperature_enabled` | `boolean` | No | `false` | Whether to dynamically adjust temperature |
...
| `reasoning_effort` | `string` | No | `null` (unset) | Reasoning effort level: `"none"`, `"minimal"`, `"low"`, `"medium"`, `"high"`, `"xhigh"`, `"ultra"`, or `"max"`. At `"xhigh"` and above, pair with a large `max_tokens` so the model has room to think and answer |
...
| `total_cost` | `float` | Total credits charged for the run: input and output token costs plus `cost_per_agent` |
...
if response.status_code == 200:
    ...
    print(f"  Token cost: ${result['usage']['total_cost']:.4f}")
else:
...
if response.status_code == 200:
    ...
    print(f"  Token cost: ${usage.get('total_cost', 0):.4f}")
