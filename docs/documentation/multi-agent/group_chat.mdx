...
Unlike the workflow types, agents talk to each other. Every contribution lands in one shared
transcript that the others can build on or push back against.
...
Every swarm run is executed through a `SwarmRouter` with `output_type` forced to `"dict"` server-side (see `api/swarm_completions.py`), so `output` is a list of `{"role": ..., "content": ...}` conversation turns rather than a single string. `GroupChat` specifically produces a multi-turn transcript: each agent speaks in turn, sees the accumulated conversation so far (including what previous agents said), and its `content` in `output` is its contribution to that shared discussion — so, unlike `ConcurrentWorkflow`, later agents' turns are expected to reference or build on earlier ones rather than being independent.
