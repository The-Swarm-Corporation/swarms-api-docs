...
| `max_loops` | integer | Yes | Number of iterations for refinement. Must be a positive integer — omitting it or passing a value below 1 returns a `400` error |
...
| `model_name` | string | No | Model to use (e.g., "gpt-4.1", "claude-sonnet-4-20250514"); defaults to `gpt-4.1` |
| `max_loops` | integer or string | No | Max loops per agent, or `"auto"` (default: 1) |
| `max_tokens` | integer | No | Maximum tokens the agent can generate (default: 8192) |
