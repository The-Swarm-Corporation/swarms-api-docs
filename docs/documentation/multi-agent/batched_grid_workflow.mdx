...
Every agent runs every task, so the output is a matrix: one result per agent-task pair.
...
| `max_loops` | integer | No | Number of iterations for refinement (default: 1). Must be between 1 and 50 — values outside that range are rejected with a `422` validation error |
...
| `max_loops` | integer or string | No | Max loops per agent, or `"auto"` (default: 1) |
