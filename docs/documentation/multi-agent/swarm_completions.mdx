...
| `heavy_swarm_question_agent_model_name` | `string \| null` | `"gpt-5.4"` | `HeavySwarm` | Model backing the question-generation agent |
...
`swarm_type` accepts one of the following **14** values (this is the live list returned by `GET /v1/swarms/available`; see [Available Architectures](/docs/documentation/multi-agent/available-architectures) for full metadata on each):
...
| `CouncilAsAJudge` | A council evaluates and a judge rules | [Docs](/docs/documentation/multi-agent/council_as_a_judge) |
| `HeavySwarm` | Question decomposition, worker agents, synthesis | [Docs](/docs/documentation/multi-agent/heavy_swarm) |
| `LLMCouncil` | Council deliberates, a chairman synthesizes | [Docs](/docs/documentation/multi-agent/llm_council) |
| `DebateWithJudge` | Pro and Con debate, a judge synthesizes | [Docs](/docs/documentation/multi-agent/debate_with_judge) |
| `RoundRobin` | Agents take turns in a fixed rotation | [Docs](/docs/documentation/multi-agent/round_robin) |
| `PlannerWorkerSwarm` | Planner decomposes, workers pull from a shared queue | [Docs](/docs/documentation/multi-agent/planner_worker_swarm) |

<Note>
`"auto"` and `"SpreadSheetSwarm"` are rejected with a `400` ("not supported currently") even though older documentation may reference them. `BatchedGridWorkflow` and `GraphWorkflow` are **not** `swarm_type` values at all — they are separate, premium-only endpoints: [`/v1/batched-grid-workflow/completions`](/docs/documentation/multi-agent/batched_grid_workflow) and [`/v1/graph-workflow/completions`](/docs/documentation/multi-agent/graph_workflow).
</Note>
...
| `mcp_url` | `string \| MCPConnection \| null` | — | A single MCP server: a URL string for an unauthenticated server, or a connection object for auth/transport |
| `mcp_config` | `MCPConnection \| null` | — | A single MCP connection. See [MCPConnection](#mcpconnection) |
| `mcp_configs` | `array[MCPConnection] \| null` | — | Several MCP connections at once |
| `mcp_urls` | `array[string \| MCPConnection] \| null` | — | Several MCP servers. Each entry is a URL string or a connection object; tools from every server are combined |
...
The first nine fields below are always present in a successful response. `tasks` and `messages` are **conditionally present**: the server echoes each one back only if the request supplied it — `tasks` appears when the request's `tasks` array was not null, and `messages` appears when the request's `messages` field was not null. Neither key is present at all on a plain `task`-only request.
...
| `number_of_agents` | `integer \| null` | Number of agents in the swarm. Counted from the request's `agents` array (`0` for architectures such as `HeavySwarm` that can build their own team) |
| `execution_time` | `number \| null` | Wall-clock execution time in seconds |
| `usage` | `object \| null` | Token counts and cost breakdown |
| `tasks` | `array[string] \| null` | Present only when the request set `tasks`. Echoes the batch of tasks the request carried, so a caller can match results to what it sent |
| `messages` | `array[object] \| object \| null` | Present only when the request set `messages`. Echoes the message history the request carried |
...
`tasks` and `messages` appear only when the request supplied them, for example a batch-style run that sets `tasks` (an array of task strings, run via `SwarmRouter.batch_run`) instead of a single `task`:

```json
{
  "job_id": "swarm-e5f6g7h8",
  "status": "success",
  "swarm_name": "Batch Analysis",
  "description": "Multiple independent analyses in one call",
  "swarm_type": "ConcurrentWorkflow",
  "output": ["...", "..."],
  "number_of_agents": 2,
  "execution_time": 12.1,
  "usage": { "input_tokens": 210, "output_tokens": 900, "total_tokens": 1110 },
  "tasks": ["Summarize Q1 earnings", "Summarize Q2 earnings"]
}
```

<Warning>
`output` is untyped in the schema because each architecture returns a different structure — `SequentialWorkflow` returns an ordered conversation, `MajorityVoting` returns votes plus a verdict. Branch on `swarm_type` rather than assuming a fixed shape. All swarm types are executed through a `SwarmRouter` that always forces `output_type="dict"` server-side, so `output` is typically a list of `{"role": ..., "content": ...}` turns — the order and the meaning of each `role` depends on the coordination pattern (see each architecture's page).
</Warning>
