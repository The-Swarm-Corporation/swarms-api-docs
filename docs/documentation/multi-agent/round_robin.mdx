---
title: "RoundRobin"
description: "Deterministic round-robin task distribution where agents take turns collaborating with full conversation context, cycling through the roster once per loop"
---
...
```mermaid
flowchart LR
    T["Task"] --> A["Turn 1"] --> B["Turn 2"] --> C["Turn 3"] --> O["Result"]
    C -.->|"next loop, same order"| A
```

Agents take turns in the order they appear in `agents`, each reading the full conversation so
