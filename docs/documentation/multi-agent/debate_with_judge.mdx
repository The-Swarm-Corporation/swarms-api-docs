...
flowchart TD
    ...
    P --> J["Judge"]
    C --> J
    J --> S["Refined synthesis"]
    S -.->|"next loop"| P
    S -.->|"next loop"| C
```

Pro and Con argue, the Judge evaluates both and synthesizes. Each loop restarts the debate
from that synthesis, so the arguments sharpen with every round.
...
```json
{
    ...
    "usage": {
        ...
        "billing_info": {
            "cost_breakdown": {
                "agent_cost": 0.03,
                "input_token_cost": 0.000247,
                "output_token_cost": 0.0333,
                "token_counts": {
