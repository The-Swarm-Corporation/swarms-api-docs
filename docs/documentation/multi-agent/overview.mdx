...
| `reasoning_effort` | `string` | No | - (unset) | The effort to put into reasoning. Options: `'none'`, `'minimal'`, `'low'`, `'medium'`, `'high'`, `'xhigh'`, `'ultra'`, `'max'`. Used with reasoning-enabled models. For Claude 5-family models (`claude-sonnet-5`, `claude-opus-5`, `claude-fable-5`) reasoning parameters are currently ignored and the agent runs without extended thinking. |
...
| `output` | `any` | The output of the swarm. Can be a string, array, or object depending on the swarm type. |
