...
The shape comes from `rearrange_flow`. Above is `"Researcher -> Writer, Editor"`: `->` is a
handoff to the next step, and a comma runs agents in parallel within the same step.
...
curl -X POST "https://api.swarms.world/v1/swarm/completions" \
  ...
  -d '{
    ...
    "rearrange_flow": "Research Specialist -> Technical Writer -> Finance Expert -> Editor",
...
type SwarmConfig struct {
    ...
    RearrangeFlow string  `json:"rearrange_flow"`
...
func main() {
    ...
    swarmConfig := SwarmConfig{
        ...
        RearrangeFlow: "Research Specialist -> Technical Writer -> Finance Expert -> Editor",
        Task:          "Create a comprehensive technical blog post about machine learning in finance, with the ability to reassign agent roles based on content needs",
        Agents: []Agent{
            {
...
```json
{
    ...
    "usage": {
        ...
        "billing_info": {
            "cost_breakdown": {
                "agent_cost": 0.04,
                "input_token_cost": 0.000146,
                "output_token_cost": 0.0259,
                "token_counts": {
