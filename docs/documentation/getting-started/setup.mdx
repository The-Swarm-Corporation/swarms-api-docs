...
| **Method**           | HTTP methods supported                                                          | `GET`, `POST`                                | Every current endpoint is either `GET` (read-only) or `POST` (completions/workflows) |
| **Request Body**     | JSON payload for requests (for POST)                                            | `{"key": "value"}`                           | Must conform to `application/json` content type                           |
