...
**Python (official `swarms-client` SDK):**

```bash
pip install swarms-client
```

The [`swarms-client`](https://pypi.org/project/swarms-client/) package wraps every endpoint on this page (`client.agent.run(...)`, `client.swarms.run(...)`, etc.) so you don't have to build the raw HTTP requests yourself. This guide shows both the raw `requests` calls and, where useful, the equivalent `swarms-client` call.
...
Multi-agent swarms enable complex problem-solving by coordinating multiple AI agents. Set the `swarm_type` field on `POST /v1/swarm/completions` to choose the orchestration pattern. The API supports 14 swarm types; call `GET /v1/swarms/available` for the full, current list with metadata. Two of the most common are shown below.

#### Workflow Types

**Sequential Workflow (`SequentialWorkflow`):** Agents execute in order, with each agent building upon the previous agent's output. This is ideal for tasks requiring step-by-step processing.

**Concurrent Workflow (`ConcurrentWorkflow`):** Agents work simultaneously on the same task, providing parallel processing for faster results and diverse perspectives.

Other supported types include `GroupChat`, `MixtureOfAgents`, `MajorityVoting`, `AgentRearrange`, `MultiAgentRouter`, `HierarchicalSwarm`, `HeavySwarm`, `LLMCouncil`, `CouncilAsAJudge`, `DebateWithJudge`, `RoundRobin`, and `PlannerWorkerSwarm` — see the [Architecture](/docs/documentation/getting-started/architecture) page for a description of each.
...
<Note>
If `model_name` is omitted, the agent uses the default model, **claude-sonnet-5**. A specific set of frontier models are premium-only: `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5.6`, `claude-fable-5`, `claude-opus-5`, `claude-opus-4-8`, `gemini-3.1-pro`, `gemini-3.1-deep-think`, `xai/grok-4.5`, `xai/grok-4.5-latest`, and any model id beginning with `groq/`. Every other model id, including `gpt-4.1`, is available on the free tier. Free-tier requests for a premium-only model are rejected with a 403 and a suggested alternative such as `claude-sonnet-5`.
</Note>

#### OpenAI Models

* **gpt-4**: High-quality reasoning and complex task handling
* **gpt-4.1**: Optimized version with improved performance
* **gpt-4.1-mini**: Lightweight version for faster responses

#### Anthropic Models

* **claude-sonnet-5**: Default model used when model_name is omitted

#### Groq Models (premium only)

* **groq/llama-3.3-70b-versatile**: High-performance open-source model
* **groq/openai/gpt-oss-120b**: Large open-weight reasoning model

#### Model Selection Guidelines

* **Complex Analysis**: Use `gpt-5.5` or `claude-sonnet-5` for tasks requiring deep reasoning
...
| `POST` | `/v1/agent/batch/completions` | Batch agent completions (premium, up to 50 per request) |
...
| `GET` | `/v1/account/logs` | Retrieve your API request logs |
| `GET` | `/v1/rate/limits` | View your rate limits and usage |
| `GET` | `/v1/account/metrics/summary` | Usage metrics summary |
| `GET` | `/v1/account/premium-endpoints` | List endpoints that require Premium |
