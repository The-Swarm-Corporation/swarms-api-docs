...
The `swarm_type` field on `POST /v1/swarm/completions` accepts 14 architectures:

| Swarm Type | Description |
|---------------|-------------|
| **SequentialWorkflow** | Agents work in order, each building on the previous output |
| **ConcurrentWorkflow** | Agents work simultaneously on the same task |
| **HierarchicalSwarm** | A director agent delegates to worker agents |
| **MultiAgentRouter** | Intelligent task distribution based on agent capabilities |
| **MixtureOfAgents** | Diverse specialist agents combined for a single task |
| **MajorityVoting** | Consensus-based decision making across agents |
| **AgentRearrange** | Dynamic agent reconfiguration via a flow expression |
| **GroupChat** | Agents converse with one another to reach an outcome |
| **CouncilAsAJudge** | A panel of agents is evaluated by a judge model |
| **HeavySwarm** | Question, worker, and synthesis agents collaborate for deep analysis |
| **LLMCouncil** | A council of models is synthesized by a chairman model |
| **DebateWithJudge** | Two agents debate; a judge delivers a verdict |
| **RoundRobin** | Agents take turns in a fixed rotation |
| **PlannerWorkerSwarm** | A planner agent decomposes the task for worker agents |

For each type's full description, category, and tuning parameters, call `GET /v1/swarms/available`.
</Accordion>

<Accordion title="What models are supported?">
The Swarms API accepts model ids from multiple providers (OpenAI, Anthropic, Google, xAI, Groq, and others) via the `model_name` field on `AgentSpec`. If `model_name` is omitted, the agent uses the default model, **claude-sonnet-5**.

**Premium-only models** (rejected on the free tier, with a suggested free-tier alternative in the error response):
...
Free-tier alternatives the API suggests in place of these: `claude-sonnet-5`, `gpt-5.5`, `gemini-3.5-flash` (and `claude-haiku-4-5`, `gpt-5.4-mini`, `gemini-3.5-flash-lite` for the `luna` tier). Every other model id, including `gpt-4.1`, is available on the free tier.
...
| **Prompt Length** | 200,000 | 2,000,000 | Per request |
| **Batch Size** | 50 | 50 | Per request |

The batch size limit of 50 applies to all tiers.
...
<Accordion title="Which endpoints require a Premium subscription?">
Exactly five endpoints require a Pro, Ultra, or Premium subscription:

| Endpoint | Feature |
|----------|---------|
| `POST /v1/agent/batch/completions` | Batch Agent Completions |
| `POST /v1/swarm/batch/completions` | Batch Swarm Completions |
| `POST /v1/reasoning-agent/completions` | Reasoning Agent Completions |
| `POST /v1/batched-grid-workflow/completions` | Batched Grid Workflows |
| `POST /v1/graph-workflow/completions` | Graph Workflows |

Call `GET /v1/account/premium-endpoints` to fetch this list live, along with an upgrade link. All billable endpoints (including the free-tier ones above plus `/v1/swarm/completions`, `/v1/chat/completions`, and `/v1/agent/completions`) additionally require a total credit balance greater than \$1.00.
</Accordion>
