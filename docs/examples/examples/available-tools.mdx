...
Enable a tool on an agent by adding its name to the top-level `tools_enabled` field of the request (alongside `agent_config` and `task`). Both tools can be enabled in the same request — each name present in `tools_enabled` adds its tool to the agent.
