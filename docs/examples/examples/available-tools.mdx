...
Enable a tool on an agent by adding its name to the top-level `tools_enabled` field of the request (alongside `agent_config` and `task`). Only one of the two tools is applied per request — if `auto_search` is present it takes precedence over `web_scraper`.
