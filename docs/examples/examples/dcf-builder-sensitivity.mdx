...
<Info>
`/v1/agent/batch/completions` and `/v1/swarm/batch/completions` are Premium-tier features, and `claude-opus-4-8` is a premium-only model — the free tier rejects it with suggested alternatives such as `claude-sonnet-5`. Note the night-mode discount applies only to `/v1/swarm/completions` and `/v1/swarm/batch/completions` — the Assumption Reasoner's `/v1/agent/batch/completions` call is billed at the standard rate regardless of time of day. Pair the SequentialWorkflow stage (Step 6, below) with the [Night-Mode Pricing Strategy](/docs/guides/guides/night-mode-pricing-strategy) — running it between 8pm and 6am Pacific halves its token cost, which is part of what keeps a 50-name nightly DCF refresh under roughly \$80 of compute.
</Info>
...
These are OpenAI-format function schemas the agents call to pull data and run mechanical math. Define them once; attach via `tools_list_dictionary` to whichever agent needs them.
...
def reasoning_assumptions_agent_config(ticker: str) -> dict:
    return {
        ...
        "max_tokens": 8000,
        "temperature": 0.2,
        "tools_list_dictionary": ALL_DATA_TOOLS,
    }
```
    ...
    return {
        ...
        "agents": [
            ...
            },
            {
                ...
                "max_tokens": 3000,
                "temperature": 0.0,
                "tools_list_dictionary": MODEL_TOOLS,
            },
            {
...
The whole point. Fan the same per-ticker pipeline across 50 names as a batch. Schedule the cron at 9pm Pacific so the run lands inside the night-mode window — the SequentialWorkflow stage (`/v1/swarm/batch/completions`) bills its tokens at 50% off in that window; the `/v1/agent/batch/completions` reasoning stage bills at the standard rate either way.
...
def run_portfolio_refresh(tickers: list[str]) -> list[dict]:
    # Step 1: Reason assumptions for every ticker via the batch agent
    # endpoint. The endpoint caps each request at 50 agent completions,
    # so chunk the portfolio; results come back in input order under
    # the response's "results" key.
    ...
    for i in range(0, len(tickers), 10):
        ...
        for r in reasoning_resp.json().get("results", []):
            text = ""
            for entry in r.get("outputs", []):
                content = entry.get("content", "")
                if isinstance(content, list):
                    content = " ".join(str(c) for c in content)
                text = str(content)
...
def persist_memos(tickers: list[str], results: list[dict]) -> None:
    # Each batch item is {"status", "swarm_name", "result", "usage"} and
    # items come back in input order — the swarm name is still the safest
    # way to recover the ticker.
    date = datetime.utcnow().strftime("%Y-%m-%d")
    by_name = {r.get("swarm_name", ""): r for r in results}
    with open(f"dcf_memos_{date}.jsonl", "w") as f:
        for ticker in tickers:
            r = by_name.get(f"DCF Pipeline — {ticker}", {})
            memo = ""
            for output in r.get("result", []) or []:
                if "Memo Writer" in output.get("role", ""):
                    content = output.get("content", "")
                    if isinstance(content, list):
                        content = " ".join(str(c) for c in content)
...
<Info>
The batch lands in the database by ~5am Pacific. See [Night-Mode Pricing Strategy](/docs/guides/guides/night-mode-pricing-strategy) for how the off-peak discount works — it halves the token cost of the SequentialWorkflow stage specifically (`/v1/swarm/batch/completions`), not the Opus reasoning stage, so scheduling overnight still lowers the total bill versus running the same workload at, say, 11am Pacific.
</Info>
...
- [Night-Mode Pricing Strategy](/docs/guides/guides/night-mode-pricing-strategy) — how the off-peak discount on `/v1/swarm/completions` and `/v1/swarm/batch/completions` factors into the 50-name refresh's total cost
