...
<Note>
The response does not include a top-level `count` field — compute the number of entries from `len(logs)` (or `logs.length` in JavaScript) instead.
</Note>
            ...
            if recent_logs:
                ...
                if response_times:
                    ...
                    if avg_response_time > 10:  # More than 10 seconds average
                        print(f"🐢 Slow responses detected: {avg_response_time:.2f}s average")
