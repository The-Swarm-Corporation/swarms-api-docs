...
<Info>
This pipeline needs a Premium account for two independent reasons: `claude-opus-4-8` is a Premium-only model, and `POST /v1/graph-workflow/completions` is a Premium-only endpoint. `reasoning_effort` itself is a plain `AgentSpec` field available on any tier — it is the model choice and the endpoint that gate this example. See the [Reasoning Agents tutorial](/docs/examples/examples/reasoning-agents-tutorial) for the full primer on reasoning-effort tuning, and [upgrade your account](https://cloud.swarms.world/settings) before running this in production.
</Info>
...
| FX | `xai/grok-4.3` | Real-time edge — picks up the live tape and positioning chatter |
