...
Four workers each own one chapter of the deck. The Engagement Director is told, very explicitly, to produce a slide-by-slide outline — not a memo — but see the mechanism note below for what it actually takes to get that agent to see the other four outputs.
...
<Note>
`HierarchicalSwarm` always builds its own director from `director_model_name` (default `gpt-5.4`) that decomposes the task and dispatches it to every agent in `agents` — the Engagement Director above runs as one of the director's workers, not as a privileged synthesizer, and with the default `max_loops: 1` and parallel worker execution, no agent (Engagement Director included) sees another specialist's output before writing its own deck outline. For a guaranteed synthesis pass, either set `"max_loops": 2` on the swarm (the framework's director is handed every first-round output on its second pass and can route the synthesis to the Engagement Director by name/description), or collect the four specialist outputs client-side and issue one more `/v1/agent/completions` call using the Engagement Director's system prompt.
</Note>
