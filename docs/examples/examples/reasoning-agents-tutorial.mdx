...
`outputs` is where the reasoning content lives, and its internal shape depends on `output_type` — with `output_type: "dict-final"` it resolves to a dict containing the convergent final answer, which is what Step 5 below extracts.
