...
- The top-level `tools_enabled` list (`"auto_search"`) for live vendor lookups with citations
...
2. When web search is enabled, perform web lookups and verify vendor contact and availability information; include citation for each vendor where possible.
...
def create_agent_payload(task: str, search_enabled: bool = True) -> Dict[str, Any]:
    return {
        ...
        "task": task,
        "tools_enabled": ["auto_search"] if search_enabled else [],
    }
```
