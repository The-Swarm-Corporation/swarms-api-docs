...
- Sustained polling rate-limit math: ETH block time of 12 seconds compounds to ~7,200 swarm calls/day, blowing through the Free tier's per-day cap after a few hours
- Real-time alert routing into Slack and Discord with severity scoring, including programmatic muting on quiet hours

<Info>
Continuous block-by-block polling breaches the Free tier per-day cap within a few hours of running. Watch the `X-RateLimit-*` response headers (or poll `GET /v1/rate/limits`) so you catch a tightening window before you start dropping blocks to 429s. See the [Rate Limits documentation](/docs/documentation/resources/ratelimits) before you flip this loop on in production.
</Info>
...
Each chain monitor runs on `gpt-4.1-mini` — fast and cheap is the only thing that matters when you are firing this swarm every 12 seconds. The Movement Classifier runs on `xai/grok-4.3` for its real-time bias and willingness to commit to a classification under uncertainty. The Alert Generator runs on `claude-sonnet-5` because the natural-language quality of the Slack message is what makes the desk actually read it.
...
- Free tier caps are **100/minute**, **350/hour**, **1,200/day** — the per-day cap is breached in **4 hours**
- Paid tiers (Pro, Ultra, Premium) share **2,000/minute**, **10,000/hour**, **100,000/day**, which holds this loop
- Watch `X-RateLimit-Remaining-*` on every response (or poll `GET /v1/rate/limits`) so you can back off before a 429 costs you a block

<Info>
Live whale tracking is a Premium workload by construction. If you try to run this loop on Free you will hit 429s and miss blocks once the per-day cap is exhausted. See [Rate Limits](/docs/documentation/resources/ratelimits) for the per-tier numbers and upgrade at [https://cloud.swarms.world/settings](https://cloud.swarms.world/settings).
</Info>
...
| --- | --- | --- |
| **This stack** (gpt-4.1-mini chains + xai/grok-4.3 classifier + sonnet-5 alerter, night-mode discount applied automatically 8pm–6am Pacific) | **~\$750/mo** (~\$25/day) | Programmable layer you own, four chains, custom whale list, structured outputs to your warehouse |
...
Night-mode tip: the API applies its off-peak token-cost discount automatically between 8pm and 6am Pacific (`America/Los_Angeles`) — no request changes needed. Separately, most chains are quieter in the early-UTC-morning hours; if you want to cut call volume too (not just get the automatic discount), drop the watcher to a 60-second cadence during that quiet window.
