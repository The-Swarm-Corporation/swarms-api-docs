...
<Info>
This tutorial uses `/v1/agent/batch/completions` — the highest-throughput endpoint on the platform. It is premium-only and accepts up to 10 tasks per request, so larger workloads are submitted in chunks. For sustained batches above 1,000 rows or production SLAs, upgrade to Pro, Ultra, or Premium at [https://swarms.world/platform/account](https://swarms.world/platform/account) for access and the rate-limit headroom you need.
</Info>
...
<Note>
The batch endpoint executes each request's tasks in parallel server-side, up to 10 tasks per request. You do not need to thread the requests on your end — submit each chunk and wait.
</Note>

## Step 4: Submit Claims in Chunks

Submit in chunks of 10 — the maximum batch size the endpoint accepts per request — which also makes retries cheap if a single chunk fails.
...
def run_batch(payloads: list[dict]) -> list[dict]:
    ...
    # Response shape: {"batch_id", "total_requests", "execution_time",
    # "timestamp", "results": [...]} — the per-task outputs live in "results".
    return response.json()["results"]


def triage_dataframe(df: pd.DataFrame, chunk_size: int = 10) -> pd.DataFrame:
...
def extract_decision(result_row: dict) -> dict:
    ...
    # Each item in "results" is an AgentCompletionOutput dict — the agent's
    # text lives under 'outputs'. Keep a fallback to 'output' for robustness.
