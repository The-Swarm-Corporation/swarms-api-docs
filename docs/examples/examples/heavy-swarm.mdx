...
HeavySwarm creates its own specialized agents internally — pass an empty `agents` array and configure the swarm using HeavySwarm-specific parameters:

```python
def run_heavy_analysis(task: str, heavy_swarm_max_loops: int = 1) -> dict:
    ...
    swarm_config = {
        ...
        "task": task,
        "agents": [],
        "heavy_swarm_max_loops": heavy_swarm_max_loops,
...
<Warning>
For `HeavySwarm`, the top-level `max_loops` field is not used - it only controls how many worker agents you supply (irrelevant here since `agents` is empty). Use `heavy_swarm_max_loops` to control HeavySwarm's own iteration count.
</Warning>

<Note>
`heavy_swarm_variant` picks an entirely different agent roster, not just a size knob:
- `"default"` (the default) — 5 agents: Question Generator, `Research-Agent`, `Analysis-Agent`, `Alternatives-Agent`, `Verification-Agent`, then a Synthesis Agent. This is what the rest of this page demonstrates.
- `"medium"` — 4 Grok-backed agents (a Captain plus Harper, Benjamin, and Lucas).
- `"heavy"` — 16 agents (a Grok lead plus 15 domain specialists).

Switching `heavy_swarm_variant` changes the `role` values in `output`, so code that matches on agent name (like `Research-Agent` below) only applies to the `"default"` variant.
</Note>
...
For tasks requiring deeper analysis, increase `heavy_swarm_max_loops`. Each iteration builds on the previous loop's synthesized result:
