...
Run multiple debate rounds where the judge's feedback helps both sides sharpen their arguments. Each loop runs Pro, then Con, then Judge in order, feeding the judge's synthesis into the next round as the new topic - `output` will contain `3 * max_loops` entries, with role names repeating per round:
