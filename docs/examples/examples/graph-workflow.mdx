...
# Display cost info (the graph-workflow endpoint returns a flat `usage`
# object with `token_cost`, not the nested `billing_info` shape used by
# /v1/swarm/completions)
usage = result.get('usage', {})
if 'token_cost' in usage:
    print(f"\nTotal cost: ${usage['token_cost']:.4f}")
else:
