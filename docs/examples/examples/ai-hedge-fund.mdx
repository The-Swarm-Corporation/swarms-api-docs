...
<Info>
`HierarchicalSwarm` via `/v1/swarm/completions` (Step 3) works on any tier with a positive credit balance. `/v1/swarm/batch/completions` (Step 4) is a **premium-only** endpoint — it requires an active premium subscription. Upgrade at [https://cloud.swarms.world/settings](https://cloud.swarms.world/settings) to unlock batch execution for overnight jobs across hundreds of tickers.
</Info>
...
<Warning>
**Premium Tier Required**: The `/v1/swarm/batch/completions` endpoint is restricted to Pro, Ultra, and Premium plan subscribers. Free tier users will receive a 403 error. [See Premium Endpoints](/docs/documentation/resources/premium-endpoints).
</Warning>
...
with open("morning_notes.jsonl", "w") as f:
    for ticker, result in zip(WATCHLIST, results):
        # Extract the Portfolio Manager's final call. Batch items are shaped
        # {"status", "swarm_name", "result", "usage"} — the conversation lives
        # under "result", not "output".
        pm_call = next(
            (o["content"] for o in result.get("result", []) if "Portfolio Manager" in o.get("role", "")),
            "",
        )
