  ...
  <Tab title="Python">
    ...
    payload = {
        "agent_config": {
            ...
            "max_tokens": 4096,
            "temperature": 0.5,
            "mcp_url": "https://github.com/mcp"  # Your MCP server URL
        },
...
`mcp_urls` takes several MCP servers directly as a list — each entry is a URL string or an `MCPConnection` object — and tools from every server are combined for the agent:
...
<Note>
`mcp_urls` (and `mcp_configs`, its `MCPConnection`-object counterpart) is a plain **list**, not an object with a `connections` key. Each list entry can be a bare URL string or a full `MCPConnection` object (see [Single MCP with Advanced Config](#single-mcp-with-advanced-config) below) if you need per-server auth or transport settings.
</Note>
