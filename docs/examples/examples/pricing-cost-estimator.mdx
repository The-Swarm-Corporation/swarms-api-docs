...
- **Estimate** costs for:
  ...
  - **Night-time discount** multiplier, auto-detected the same way the API applies it

<Note>
The night-time discount is not a client-side guess — it mirrors `api/deductions_utils.py` exactly: off-peak is **8:00 PM through 5:59 AM in `America/Los_Angeles`** (`now.hour >= 20 or now.hour < 6`), and the discount multiplies the swarm completion **token cost only** — the per-agent fee is always charged in full, at any hour.
</Note>
...
<Tabs>
  <Tab title="Python">
    ...
    # Mirrors api/deductions_utils.py exactly: off-peak is 8 PM through
    # 5:59 AM in the pricing timezone (America/Los_Angeles).
    PRICING_TIMEZONE = "America/Los_Angeles"
    NIGHT_START_HOUR = 20
    NIGHT_END_HOUR = 6

    def is_night_time(now: Optional[datetime] = None) -> bool:
        """Whether the off-peak discount applies right now (server-side rule)."""
        if now is None:
    ...
    def estimate_cost(usage_pricing: dict, *, swarm_input_tokens: int = 0,
                      ...
                      night_time: Optional[bool] = None) -> float:
        up = usage_pricing

        # Auto-detect off-peak the same way the server does, unless the
        # caller overrides it (e.g. to price a job scheduled for later).
        ...
        # Swarm completions (the night-time discount applies only to
        # swarm input/output token costs, not the per-agent fee — see
        # charge_swarm_usage in api/deductions_utils.py)
        swarm_token_cost = (
        ...
        if night_time:
            swarm_token_cost *= up["night_time_discount"]
        cost += swarm_token_cost
    ...
    if __name__ == "__main__":
        ...
        est = estimate_cost(
            ...
            searches=20,
            scrapes=5,
            # night_time=None (default) auto-detects via is_night_time();
            # pass True/False explicitly to price a different time window.
        )
  ...
  <Tab title="TypeScript">
    ...
    function estimateCost(
      ...
      // Swarm completions (the night-time discount applies only to
      // swarm input/output token costs, not the per-agent fee)
      let swarmTokenCost =
        tokensToM(swarmInputTokens) * up.swarm_completions_input_cost_per_1m +
        tokensToM(swarmOutputTokens) * up.swarm_completions_output_cost_per_1m
      if (nightTime) swarmTokenCost *= up.night_time_discount
      cost += swarmTokenCost
  ...
  <Tab title="Rust">
    ...
    fn estimate_cost(up: &UsagePricingModel) -> f64 {
        ...
        // Swarm completions (the night-time discount applies only to
        // swarm input/output token costs, not the per-agent fee)
        let mut swarm_token_cost = tokens_to_m(swarm_input_tokens)
            * up.swarm_completions_input_cost_per_1m
            + tokens_to_m(swarm_output_tokens) * up.swarm_completions_output_cost_per_1m;
        if night_time {
            swarm_token_cost *= up.night_time_discount;
        }
  ...
  <Tab title="Go">
    ...
    func estimateCost(up UsagePricingModel) float64 {
        ...
        if nightTime {
            swarmTokenCost *= up.NightTimeDiscount
        }
        cost += swarmTokenCost
