...
| `chunk` | Many per agent — one per token (or small group) | `agent`, `content` (the token text) |
| `agent_end` | Once per agent, when it finishes | `agent` (final per-agent content is not included in this event — collect it from the `chunk` events) |
| `usage` | Once, near the end | `input_tokens`, `output_tokens`, `total_tokens`, `billing_info` |
| `end` | Once, terminal | `execution_time`, `status` (does not include the aggregated output — collect it from the `chunk`/`agent_end` events as you stream) |
