...
<Info>
This pipeline runs continuously against a 24/7 news firehose at sustained volume. The Free tier's request-rate ceiling (100/minute, 350/hour, 1,200/day) is easy to hit once the overlap gate is firing dozens of swarm calls a day on top of your other traffic; Premium raises that to 2,000/minute, 10,000/hour, 100,000/day. Upgrade at [https://cloud.swarms.world/settings](https://cloud.swarms.world/settings). See the [Production Observability Guide](/docs/guides/guides/production-observability) for the metrics that matter when news latency is the product.
</Info>
...
`HierarchicalSwarm` supplies its own director agent — set via the SwarmSpec-level `director_model_name` field, not as an entry in `agents` — so every agent you list below is a worker the director can route to. The director runs on `claude-sonnet-5` because synthesizing multi-sector takes into a single coherent dispatch needs strong reasoning. The four sector workers run on `gpt-4.1-mini` — they are doing tight, scoped, per-sector reasoning over a single article, which is exactly the workload that cheap fast models eat for breakfast. The Trade Idea Synthesizer goes back to `claude-sonnet-5` because the final thesis is the artifact that lands in front of the PM.
...
def build_news_swarm(article_text: str, article_url: str, overlap: list[str]) -> dict:
    return {
        ...
        "description": (
            "The director routes the article across sector workers and a Trade Idea "
            "Synthesizer to emit portfolio-overlapped trade theses to Slack."
        ),
        "swarm_type": "HierarchicalSwarm",
        "director_model_name": "claude-sonnet-5",
        "director_settings": {"temperature": 0.2, "max_tokens": 4096},
        "max_loops": 1,
        "task": (
            "You are directing a multi-sector trading desk's response to a live news "
            "article. Route the article to the relevant sector workers (Tech, Energy, "
            "Healthcare, Financials), collect their per-ticker takes, and hand a clean, "
            "deduplicated package to the Trade Idea Synthesizer, which posts one trade "
            "thesis per overlapping ticker to Slack.\n\n"
...
def fire_swarm(article_text: str, article_url: str, overlap: list[str]) -> int:
    ...
    # The Synthesizer's final message is either a text summary
    # (tool_call_summary=True, the default) or a list of raw tool-call objects
    # shaped {"function": {"name": ..., "arguments": ...}} — see the Tools
    # guide for the raw shape.
    synthesizer_output = next(
        (o for o in result.get("output", []) if "Trade Idea Synthesizer" in o.get("role", "")),
        {},
    )
    content = synthesizer_output.get("content", "")
    if isinstance(content, list):
        return len([
            c for c in content
            if isinstance(c, dict)
            and c.get("function", {}).get("name") == "post_trade_thesis_to_slack"
        ])
    return str(content).count("post_trade_thesis_to_slack")
```
...
At sustained continuous volume, observability is not optional — it is the SLA. Track the metrics that matter when news latency *is* the product in your own logging layer: articles ingested per minute, articles passing the overlap gate, swarm calls fired, theses posted to Slack, p50/p99 article-to-Slack latency, and — most importantly — the **missed-news SLA**: any article that overlapped the book but failed to produce a thesis within the 60-second budget. `GET /v1/account/logs` gives you the request-level audit trail and `GET /v1/rate/limits` gives you the live rate-limit window, both of which feed straight into that dashboard; Premium's higher per-minute/hour/day ceiling (see above) is what keeps the missed-news SLA inside budget at peak earnings-week volume. See the [Production Observability Guide](/docs/guides/guides/production-observability) for the full set of metrics and alerting recipes.
