...
- The rate-limit math: a 5-minute cadence is 12 requests per hour, comfortably inside the Free tier's 350/hour cap — this workload doesn't require Premium on request volume alone
...
Each pass launches the swarm, which kicks off 4 agents and the underlying tool calls. That is roughly 312 agent-runs per hour during market hours, but rate limits count requests, not agents: 12 requests/hour sits well inside the Free tier's 350/hour and 1,200/day caps. This single-watchlist cadence does not require Premium on request volume — if you fan this out across multiple watchlists or tighten the cadence below 5 minutes, watch your `X-RateLimit-Remaining-*` headers so you know how much headroom is left before Premium's higher per-minute/hour/day caps (2,000/10,000/100,000) become necessary. Full numbers at [Rate Limits](/docs/documentation/resources/ratelimits).

<Info>
A 5-minute window that completes at minute 6 is a missed window — the next cron fires before the last alert lands in Slack. That is a latency/tuning problem (model choice, `max_tokens`, tool round-trips), not a rate-limit one at this cadence; watch wall time on each pass and cut `max_tokens` on the specialist agents if a pass runs long.
</Info>
