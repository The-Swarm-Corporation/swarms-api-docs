...
- The rate-limit math: a 5-minute cadence is 12 requests per hour, comfortably inside the Free tier's 350/hour cap — what Premium buys you here is the priority queue, not raw call volume
...
Each pass launches the swarm, which kicks off 4 agents and the underlying tool calls. That is roughly 312 agent-runs per hour during market hours, but rate limits count requests, not agents: 12 requests/hour sits well inside the Free tier's 350/hour and 1,200/day caps. Production still wants **Premium** for the priority processing queue and the headroom not to drop a window when the chain endpoint is slow. Full numbers at [Rate Limits](/docs/documentation/resources/ratelimits).
