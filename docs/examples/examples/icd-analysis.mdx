...
<Warning>
All three agents below use `groq/openai/gpt-oss-120b`. Any `groq/*` model is premium-only — a free-tier API key is rejected on these agents (`claude-sonnet-5`, `gpt-5.5`, and `gemini-3.5-flash` are the suggested free-tier alternatives). You need an active Pro, Ultra, or Premium subscription to run this example as written. Manage your plan at [https://swarms.world/platform/account](https://swarms.world/platform/account).
</Warning>
