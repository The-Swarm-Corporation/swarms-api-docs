...
Unlike [agent handoffs](/docs/examples/examples/agent-handoffs) where specialist agents are pre-defined in your request, sub-agents are **created dynamically at runtime** by the coordinator. The coordinator decides how many agents to create and what each one specializes in.
...
if result.get("success"):
    ...
    output_text = str(result["outputs"])
    print(f"Output:\n{output_text[:2000]}...")
else:
...
By default, an agent with `max_loops="auto"` can use every safe autonomous tool, including `create_sub_agent` and `assign_task`. To limit the coordinator to only planning and delegation (no file I/O), set `selected_tools` on `agent_config`:

```python
"agent_config": {
    ...,
    "max_loops": "auto",
    "selected_tools": ["create_plan", "think", "create_sub_agent", "assign_task", "subtask_done", "complete_task"],
}
```
