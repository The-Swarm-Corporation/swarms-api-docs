...
<Warning>
**Batch size limit**: `/v1/swarm/batch/completions` accepts at most **50** `SwarmSpec` items per request — this tutorial is deliberately sized to that ceiling. A request with more than 50 items is rejected with an HTTP 422 validation error before any swarm runs. If your watchlist grows past 50, chunk it the same way [Batch Agent Completions at Scale](/docs/examples/examples/batch-agent-scale-tutorial) chunks leads, and submit the chunks concurrently.
</Warning>
...
def persist_results(results: list[dict], watchlist: list[tuple[str, str]]) -> Path:
    ...
    for (name, ticker), swarm_result in zip(watchlist, results):
        ...
        cost = usage.get("billing_info", {}).get("total_cost", 0.0)
...
def retryable(swarm_result: dict) -> bool:
    return swarm_result.get("status") != "success"
...
if failed:
    ...
    # merge retry_results back into results by swarm_name / index
```
