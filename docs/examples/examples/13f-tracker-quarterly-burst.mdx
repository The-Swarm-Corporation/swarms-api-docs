...
- The batch volume math for the quarterly burst: ~5,000 filings dropping in 48 hours, dispatched concurrently against `/v1/graph-workflow/completions`
- Model diversification across the DAG: `gpt-4.1-mini` for mechanical parsing, `gpt-4.1` for aggregation, `claude-sonnet-5` for the synthesis layer
...
<Info>
This tutorial leans on the graph workflow endpoint (`/v1/graph-workflow/completions`), dispatched concurrently across the filing list for the burst — a **premium-only** endpoint (Pro, Ultra, Premium). Note that the API's night-mode discount (see [Night-Mode Pricing Strategy](/docs/guides/guides/night-mode-pricing-strategy)) applies only to `/v1/swarm/completions` and `/v1/swarm/batch/completions` — it does **not** apply to `/v1/graph-workflow/completions`, so running this burst overnight does not itself lower the bill. The reasons to fire it overnight here are operational: EDGAR's after-hours filing pattern and your own concurrency/rate-limit headroom, not a pricing discount.
</Info>
...
- **Theme synthesizer** → `claude-sonnet-5` — produces the journalist-grade narrative the brief is built around.
...
print(f"\nToken cost: {result['usage']['total_cost']:.4f} credits")
...
**The math.** A single 5-node GraphWorkflow per filing — three lean parsers on `gpt-4.1-mini`, one aggregator on `gpt-4.1`, one synthesizer on `claude-sonnet-5` — comes out to roughly **\$0.04 per filing**. Across the full quarterly burst:

| Volume | Per-filing cost | Total |
| --- | --- | --- |
| 5,000 13F-HR filings | ~\$0.04 | **~\$200** |

A takeout dinner gets you the entire quarter's 13F coverage. Run four times a year and your annual budget for the program is roughly **\$800** in API cost.

<Info>
`/v1/graph-workflow/completions` token costs do not get the night-mode discount (that applies only to `/v1/swarm/completions` and `/v1/swarm/batch/completions` — see [Night-Mode Pricing Strategy](/docs/guides/guides/night-mode-pricing-strategy)). Firing the burst overnight is still worth doing here to stay clear of daytime rate-limit and concurrency contention on other production workloads — see [Rate Limits](/docs/documentation/resources/ratelimits) if you need to chunk a larger batch across multiple windows.
</Info>
...
| **GraphWorkflow batch burst, 5,000 funds** | **~1 hour server-side** | **~\$200** | **~\$800** |
...
- [Night-Mode Pricing Strategy](/docs/guides/guides/night-mode-pricing-strategy) for how the off-peak discount works on `/v1/swarm/completions` — useful if you rebuild a stage of this pipeline on the swarm endpoint instead of GraphWorkflow
