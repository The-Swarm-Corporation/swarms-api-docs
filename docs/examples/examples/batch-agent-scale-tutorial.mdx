...
`/v1/agent/batch/completions` enforces a **hard server-side cap of 10 items per request** — send more than 10 `AgentCompletion` objects in one call and you get back an HTTP 400 ("BATCH SIZE EXCEEDED"). For 10,000 leads that means 1,000 chunked requests, so submit the chunks concurrently with a small thread pool instead of one at a time, or wall-clock time balloons.

```python
import concurrent.futures

CHUNK_SIZE = 10  # hard cap enforced by the server; larger chunks return HTTP 400
...
<Info>
Each request is capped at 10 items, but the server processes those 10 concurrently, and Pro/Ultra/Premium keys allow up to 2,000 requests per minute — so a pool of 20-30 worker threads submitting 10-item chunks in parallel gets you back to job-like throughput without tripping the per-request limit or the per-minute rate limit.
</Info>
...
| **Batch endpoint, 1,000 chunks of 10 (25 concurrent workers)** | **~15-25 minutes** | **~\$30** | **\$30 + 15-25 minutes** |
