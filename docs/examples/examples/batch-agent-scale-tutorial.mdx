...
- How to score or triage tens of thousands of records with `/v1/agent/batch/completions`
...
CHUNK_SIZE = 50  # hard cap enforced by the server; larger chunks return HTTP 422
...
def run_chunk(chunk: list[dict]) -> list[dict]:
    ...
    # The endpoint returns an envelope, not a bare list:
    # {"batch_id", "total_requests", "results", "execution_time", "timestamp"}
    return response.json()["results"]


chunks = [
    batch_items[start : start + CHUNK_SIZE]
...
for lead, result in zip(leads, all_results):
    # A failed item has "success": False and no "outputs" — the batch
    # endpoint reports failures per item instead of raising for the request.
    if not result.get("success", True):
        tiers["D"].append({**lead, "score": 0, "tier": "D", "reason": f"agent_error: {result.get('detail')}"})
        continue
...
<Note>
Each result carries its own `success` flag — a failed item returns `{"success": false, "name": ..., "detail": ..., "status_code": ...}` in its slot, not an `outputs` field, and does not fail the rest of the batch. Check `success` before parsing `outputs`. The exact shape of a *successful* output still depends on the model and whether the agent returns structured content, so also wrap the JSON parse in a try/except and dump unparseable rows to a review queue — never let one bad row halt a 10k-lead pipeline.
</Note>
...
| **Batch endpoint, 200 chunks of 50 (25 concurrent workers)** | **~15-25 minutes** | **~\$30** | **\$30 + 15-25 minutes** |
