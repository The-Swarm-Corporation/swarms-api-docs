...
|---|---|
| `agent` | An AI agent listed at `swarms.world/agent/{id}` |
| `prompt` | A system prompt listed at `swarms.world/prompt/{id}` |
| `tool` | A tool listed at `swarms.world/tool/{id}` |
...
|---|---|---|
| `401` | `"Authorization header with a Bearer token is required"` | `Authorization` header is missing or malformed. |
