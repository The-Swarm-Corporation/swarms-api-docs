...
description: "Discover community-built agents, prompts, and tools on the Swarms Marketplace."
---
...
Curated collections that package agents and prompts into a single, shareable toolkit:

- **Research Stacks**: Research, summarization, and citation agents and prompts bundled together

- **Team Onboarding Kits**: A vetted set of tools to get new teammates productive fast

- **Custom Prompts**: Inline prompts written directly into the bundle, without a standalone listing

- **Related Links**: Supporting docs, GitHub repos, or demos alongside the bundled items

See the [Bundles](/docs/marketplace/bundles) guide for details, or publish one programmatically via the [Bundles API](/docs/marketplace/bundles-api).
...
3. **Enhance Your Submission:**
   - **Add Image**: Upload a visual representation (images up to 5MB, GIFs up to 35MB, videos up to 50MB)
...
Bundles package existing agents and prompts—plus your own custom prompts—into a single reusable toolkit.

**Step-by-Step Process:**

1. **Click "Add Bundle"** from the marketplace interface (or select "Bundle" as the product type at [swarms.world/publish](https://swarms.world/publish))
2. **Fill Required Fields:**

   - **Name**: Descriptive title (min. 2 characters)

   - **Description**: What the bundle is for and who it's for

3. **Add Items**: Reference existing agents/prompts by marketplace URL, or write custom prompts inline (name, description, content). A bundle needs at least one item.
4. **Enhance Your Submission:** Add a cover image, comma-separated tags, and optional related links (docs, GitHub, demos)
5. **Submit**: Review and publish—bundles are free to create and free to access

Bundles can also be published programmatically via the [Bundles API](/docs/marketplace/bundles-api) (`POST /api/v1/publish/bundle`), which is handy for CI pipelines or bulk-publishing curated kits.
...
- **File Size**: Images up to 5MB, GIFs up to 35MB, videos up to 50MB
...
**Ready to contribute?** Visit `https://swarms.world` and click "Add Prompt," "Add Agent," "Add Tool," or "Add Bundle" to share your innovation with the world.
