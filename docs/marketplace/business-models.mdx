...
| **How the creator earns** | — | 90% of each sale in SOL | 90% of each sale, paid out to bank | Bonding curve trading fees on every buy/sell | **Double** bonding curve fees on every trade | Token demand + trading fees |
...
| **Payouts** | — | Instant SOL to your wallet | Automatic bank payouts via Stripe | Claimable creator fees | Claimable creator fees (2×) | Claimable creator fees |
| **Extra perks** | Reach & reputation | — | 100+ countries, multi-currency | Price discovery, community ownership | Frenzy leaderboard + FRENZY badge | Built-in Buy $TICKER widget, creator bypass |
...
The same fixed-price model, powered by Stripe. Buyers in **100+ countries** check out with cards, **Apple Pay, Google Pay, Alipay, crypto**, and more, in 100+ currencies — no wallet required. Sellers onboard once via the [Seller tab](https://swarms.world/platform/account?tab=seller) and receive automatic bank payouts of 90% per sale; the platform fee is the same 10% as crypto. See the [Fiat Payments Overview](/docs/marketplace/fiat-payments).
