...
| `fee_selection` | string | No | `'frenzy'` \| `'market'`; fee mode for the token launch |
| `quote_mint` | string | No | `'SOL'` \| `'USDC'`; quote currency for the bonding curve (default `SOL`) |
...
<ParamField body="username" type="string">
  Return the agents owned by this username
</ParamField>

<ParamField body="agent_name" type="string">
  Match agent names (substring / slug match)
</ParamField>
...
- **Daily Limit**: 3 paid agents per user per day (15 for VIP accounts); 500 free items (agents + prompts) per user per day
- **Reset Time**: Midnight
...
```json
{
  ...
  "message": "Daily limit reached: 3 paid agents per day. Resets at midnight.",
  "currentUsage": {
    "paidAgents": 3,
    "paidPrompts": 0,
    "freeContent": 0,
...
Agents failing validation get a `400 Bad Request` with detailed reasons.
