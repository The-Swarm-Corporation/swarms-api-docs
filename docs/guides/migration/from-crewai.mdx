...
| `Crew(agents, tasks, process=sequential)` | `SequentialWorkflow` via `/v1/swarm/completions` |
| `Crew(agents, tasks, process=hierarchical)` | `HierarchicalSwarm` via `/v1/swarm/completions` |
| `crew.kickoff()` | `POST /v1/swarm/completions` |
| `crew.kickoff_async()` | Same endpoint; async via `httpx.AsyncClient` |
| `@tool` / `BaseTool` | `tools_list_dictionary` (OpenAI-style function schemas) on agent spec |
| `manager_llm` | Director agent in `HierarchicalSwarm` |
| `memory=True` | Stateless per-request; add external memory for cross-session |
| `verbose=True` | Full agent outputs returned in `output` response field |
...
| Output parsing | `output_pydantic`, `output_json` | Not a dedicated field; instruct the model to return structured JSON via `system_prompt`/`task`, or define a schema in `tools_list_dictionary` |
| Rate limiting | Depends on LLM provider | Managed by the API; see [rate limits](/docs/documentation/resources/ratelimits) |
| Cost tracking | External | `usage.billing_info.total_cost` (swarm) / `usage.total_cost` (agent) in every response |
