...
| `ToolNode` / `bind_tools` | `"tools_enabled": ["auto_search", "web_scraper"]` on the request, or `tools_list_dictionary` on the agent for custom function schemas |
...
| Streaming | `astream_events()` | Set `"streaming_on": true` on the agent config (SSE response from the same endpoint) |
