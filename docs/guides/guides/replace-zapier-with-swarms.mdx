...
per_call    ≈ $0.0085  (varies with model, prompt length, and tool-schema size)
```

At ~\$0.0085 per call, **10,000 agent calls cost about \$85** — and each one is a full multi-step reasoning flow, not one Zap node. (Note: `/v1/agent/completions` charges the agent-completions token rates only — there's no separate per-agent flat fee on single-agent completions; that `$0.01`/agent fee is specific to `/v1/swarm/completions`.)

| Volume | Zapier (7-step Zap) | Swarms agent call (~\$0.0085/call) | Savings |
| --- | --- | --- | --- |
| 1,000 runs/mo | 7,000 tasks → Professional + overage | ~\$8.50 | ~89% |
| 5,000 runs/mo | Team / Company tier | ~\$42.50 | ~75% |
| 10,000 runs/mo | Enterprise quote (4-figure) | ~\$85 | ~90% |
