...
Service homepage: [https://mcp-gateway.swarms.world/](https://mcp-gateway.swarms.world/)

x402 clients handle 402 flows automatically (detect payment, construct headers, retry).
Provide a wallet signer via `PRIVATE_KEY` or a server wallet.
Learn more at the x402 docs.
</Info>

<Note>
This gateway (`mcp-gateway.swarms.world`) is a separate, x402-metered service from the main Swarms API (`api.swarms.world`) documented elsewhere on this site — it uses on-chain USDC micropayments instead of `x-api-key` credits.
</Note>
...
Links: service homepage — [https://mcp-gateway.swarms.world/](https://mcp-gateway.swarms.world/)
</Tip>
