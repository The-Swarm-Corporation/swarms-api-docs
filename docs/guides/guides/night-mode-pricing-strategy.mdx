...
The discount is applied server-side, in `api/deductions_utils.py`. `is_night_time()` reads the wall-clock hour in the `PRICING_TIMEZONE` constant — `America/Los_Angeles` — and `charge_swarm_usage()` (called by the swarm-completions billing path) multiplies the token components by `pricing.night_time_discount` only when that check is true:

```python
# Excerpt from api/deductions_utils.py — the off-peak check
PRICING_TIMEZONE = "America/Los_Angeles"
NIGHT_START_HOUR = 20
NIGHT_END_HOUR = 6

def is_night_time(now: Optional[datetime] = None) -> bool:
    if now is None:
        now = datetime.now(pytz.timezone(PRICING_TIMEZONE))
    return now.hour >= NIGHT_START_HOUR or now.hour < NIGHT_END_HOUR

# In charge_swarm_usage(), once night_time is resolved:
if night_time:
    input_cost *= pricing.night_time_discount   # 0.50 = 50% off
    output_cost *= pricing.night_time_discount  # 0.50 = 50% off
# agent_cost is added to total_cost after this block, so it is never discounted.
```
...
For teams already running Airflow, this is the production-grade pattern. The DAG runs once per night at 9 PM PT, fans out to N batch jobs in parallel via `/v1/swarm/batch/completions`, and writes a cost-discount audit so you can prove the 50% landed.

<Note>
`/v1/swarm/batch/completions` is a premium-only endpoint (`GET /v1/account/premium-endpoints` lists it alongside `/v1/agent/batch/completions`, `/v1/reasoning-agent/completions`, `/v1/batched-grid-workflow/completions`, and `/v1/graph-workflow/completions`). If your account is on the free tier, submit the jobs in `JOBS` as separate `/v1/swarm/completions` calls instead, as in Recipe 1.
</Note>
...
def nightly_swarm_batch():
    ...
    @task
    def audit_discount(batch_result: list):
        ...
        return f"OK — all {len(batch_result)} runs got the 50% discount."
