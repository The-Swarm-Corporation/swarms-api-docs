...
print(health_status.model_dump_json(indent=4))

# List available models
available_models = client.models.list_available()
print("\nAvailable Models:")
print(available_models.model_dump_json(indent=4))

# Check current rate limits — this call returns a plain dict, not a model,
# so json.dumps works directly here.
...
print("\nSwarm Availability:")
print(swarm_availability.model_dump_json(indent=4))
```
...
<Note>
`client.health.check()`, `client.models.list_available()`, and `client.swarms.check_available()` return typed Pydantic response objects — use `.model_dump()` / `.model_dump_json()` or attribute access (`health_status.status`), not dict subscripting or `json.dumps()` directly on the object. `client.client.rate.get_limits()` is the one exception: it returns a plain `dict`.
</Note>
...
def monitor_system_status(client):
    # Check API health — HealthCheckResponse.status defaults to "ok"
    health_status = client.health.check()
    if health_status.status != "ok":
        print(f"API health issue detected: {health_status.status}")

    # Monitor rate limits — get_limits() returns a plain dict shaped like
    # RateLimitsOutput: {"rate_limits": {"minute": {...}, ...}, "limits": {...}, "tier": ..., ...}
    rate_limits = client.client.rate.get_limits()
    minute_used = rate_limits.get("rate_limits", {}).get("minute", {}).get("count", 0)
    minute_limit = rate_limits.get("limits", {}).get("maximum_requests_per_minute", 0)
    if minute_limit:
        usage_percentage = (minute_used / minute_limit) * 100
        if usage_percentage > 80:
            print(f"High API usage detected: {usage_percentage:.1f}% of the per-minute rate limit")

    # Check swarm-type listing is reachable
    availability = client.swarms.check_available()
    if not availability.success:
        print("Swarm type listing is currently unavailable")

    # client.swarms.get_logs() calls the deprecated /v1/swarm/logs alias for
    # /v1/account/logs. It only ever contains successful, billable completions —
    # rejected or failed requests never appear there, so it can't be used to
    # detect errors. Use it for a completion count / cost audit instead; see
    # the Production Observability guide for the full pattern.
    logs_response = client.swarms.get_logs()
    print(f"Logged completions on record: {logs_response.count}")
