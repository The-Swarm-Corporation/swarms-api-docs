...
- [ ] Use the request ID as the join key when you later reconcile against `/v1/account/logs`
...
- [ ] Daily export of `/v1/account/logs` to your own log lake (S3, BigQuery, etc.) — note it only contains successful, billable runs (`category="completion"`); rejected or failed requests never appear there, so capture those from the client, not from this endpoint
- [ ] Pin `model_name` per agent. Do not let your code "pick a model" at runtime in regulated workloads
- [ ] For regulated workloads, set `temperature=0` (or omit `temperature` entirely — if unset, no temperature is sent and the provider's own default applies) and `max_loops=1` for reproducibility
...
class ProductionClient:
    ...
    Features:
      ...
      - Per-request UUID for correlation against /v1/account/logs.
    ...
    def verify_in_logs(self, request_id: str) -> bool:
        """Confirm the request landed in /v1/account/logs (audit-trail check).

        Each log row's ``data`` field is the full SwarmCompletion response —
        the swarm's name comes back as ``swarm_name``, not ``name``. Only
        successful, billable runs are ever logged here, so this only confirms
        success; it can't tell you a request failed.
        """
        r = requests.get(f"{BASE_URL}/v1/account/logs", headers=HEADERS, timeout=30)
        r.raise_for_status()
        marker = request_id[:8]
        for entry in r.json().get("logs", []):
            data = entry.get("data") or {}
            swarm_name = data.get("swarm_name") or ""
            if marker in swarm_name:
                return True
        return False
```
...
if not client.verify_in_logs(result.request_id):
    log.warning("rid=%s did not appear in /v1/account/logs", result.request_id)
```
...
| `temperature` (regulated) | 0, or omit entirely for provider default | Reproducibility for audit |
...
<AccordionGroup>
  ...
  <Accordion title="Treating /v1/account/logs as real-time">
